...
with open('garbage_classifier.pkl', 'rb') as model_file:
...
# @app.route('/photos', method=['GET'])
# def show():
#     conn = sqlite3.connect('database.db')
#     c = conn.cursor()
#     c.execute("SELECT * FROM images ORDER BY id")
#     user = c.fetchall()
#     conn.close()
...
#     cursor.execute('SELECT  filename, PredictedValue FROM images ORDER BY id DESC LIMIT 10')
#     rows = cursor.fetchall()

#     conn.close()

#     # Define the column names
#     column_names = ['Delay', 'laborers']

#     # Convert the fetched rows to a list of dictionaries
#     images = [dict(zip(column_names, row)) for row in rows]

#     # Return the data as JSON
#     return jsonify({'historical_data': images})
